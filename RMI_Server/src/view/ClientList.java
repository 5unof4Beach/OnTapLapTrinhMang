...
public class ClientList extends JFrame
{
    ...
    public void addNewRows(final Answer answer) {
        ...
        final Date currentTime = new Date();
        final String dateString = formatter.format(currentTime);
        this.model.addRow(new Object[] { this.model.getRowCount() + 1, answer.getStudent().getMaSV(), answer.getStudent().getHovaten(), answer.getStudent().getIP(), answer.getStudent().getGroup(), answer.isIsAlreadyRegistration() ? "Yes" : "No", dateString });
    }
    ...
    public void updateAnswerView(final Answer answer) {
        ...
        while (it.hasNext()) {
            ++rowPos;
            final Vector next = (Vector) it.next();
